...
def main():
    ...
    Omega_range = np.linspace(0.20, 1.55, 1000)
    run_time = 10_000
    tr_time = 100
    ...
    phs.set_parameter('eps', 0.05)
    phs.evaluate(frequency, run_time, tr_time)

    plt.subplot(221)
    plt.plot(phs.x_sol[0], phs.x_sol[1], lw=1.0, color=PLOT_COLORS[1])
    plt.ylabel(r'$y$')
    plt.title(r"$\varepsilon=0.05, \Omega=0.4$")

    phs.set_parameter('eps', 0.2)
    phs.evaluate(frequency, run_time, tr_time)

    plt.subplot(222)
    plt.plot(phs.x_sol[0], phs.x_sol[1], lw=1.0, color=PLOT_COLORS[1])
    plt.title(r"$\varepsilon=0.2, \Omega=0.4$")

    phs.set_parameter('eps', 0.3)
    phs.evaluate(frequency, run_time, tr_time)
    ...
    plt.xlabel(r'$x$')
    plt.ylabel(r'$y$')
    plt.title(r"$\varepsilon=0.3, \Omega=0.4$")

    phs.set_parameter('eps', 0.4)
    phs.evaluate(frequency, run_time, tr_time)

    plt.subplot(224)
    plt.plot(phs.x_sol[0], phs.x_sol[1], lw=1.0, color=PLOT_COLORS[1])
    plt.xlabel(r'$x$')
    plt.title(r"$\varepsilon=0.4, \Omega=0.4$")

    plt.savefig("draft_plots/phase_portraits.pdf", format="pdf", 
                bbox_inches="tight")
    """
